...
lmfit_2008<- lm(log(SalePrice)~`Gr Liv Area`+`Garage Cars`+`Overall Qual`+`Year Built`,ames_raw_2008)
```
...
1. The residuals show a relatively small spread, with value closed to zero.
2. The coefficients are significant given the small value of the p-value
3. The R-squared value is 0.8232 which means 82.32% of the variance in the target variable is explained by the model.
~~~
...
1.Including all the predictors will lead to overfitting, overfit models perform well on training data but may not generalize well to new data. 
2.A simpler model with fewer predictors is often more interpretable.
3.Including all predictors may result in a computationally expensive model, especially if you have a large number of predictors.
~~~
...
I try to add the interaction terms in the model, but the p-value is always too big which makes the model worse. So I just use the model I have now which has no interaction term.
~~~
...
Your code:
```{r,echo=TRUE}
ames_raw_2008$`Garage Cars` <- replace(ames_raw_2008$`Garage Cars`, is.na(ames_raw_2008$`Garage Cars`), 0)
polyfit_2008_GrLivArea <- lm(log(SalePrice) ~ poly(`Gr Liv Area`, 3), data = ames_raw_2008)
polyfit_2008_GarageCars <- lm(log(SalePrice) ~ poly(`Garage Cars`, 3), data = ames_raw_2008)
polyfit_2008_OverallQual <- lm(log(SalePrice) ~ poly(`Overall Qual`, 3), data = ames_raw_2008)
polyfit_2008_YearBuilt <- lm(log(SalePrice) ~ poly(`Year Built`, 3), data = ames_raw_2008)

summary(polyfit_2008_GrLivArea)
summary(polyfit_2008_GarageCars)
summary(polyfit_2008_OverallQual)
summary(polyfit_2008_YearBuilt)
...
~~~
while the models for Garage Cars and Overall Qual indicate evidence of non-linear associations, the models for Gr Liv Area and Year Built suggest that a simpler linear or quadratic model might be more appropriate. 
~~~
...
```{r,echo=TRUE}
lmfit_2008<- lm(log(SalePrice)~`Gr Liv Area`+`Garage Cars`+`Overall Qual`+`Year Built`,ames_raw_2008)
predictions <- exp(predict(lmfit_2008, newdata = ames_raw_2008))

residuals <- ames_raw_2008$SalePrice - predictions

percentage_difference <- (residuals / ames_raw_2008$SalePrice) * 100

overpriced_houses <- data.frame(
  Order = ames_raw_2008$Order,
  Actual_Price = ames_raw_2008$SalePrice,
  Predicted_Price = predictions,
  Residual = residuals,
  Percentage_Difference = percentage_difference
)
overpriced_houses <- overpriced_houses[order(-overpriced_houses$Percentage_Difference), ]

top_5_overpriced_houses <- head(overpriced_houses, 5)
print(top_5_overpriced_houses)
...
We have house order number as 2257,1641,1638,2075,2309. These houses have the highest percentage differences between their actual prices and the prices predicted by your model, indicating that they are potentially overpriced according to the model's predictions.
~~~
- What are the top 5 most good deal based on your model?

Your code:
```{r,echo=TRUE}
good_deals <- overpriced_houses[order(overpriced_houses$Percentage_Difference), ]
top_5_good_deals <- head(good_deals, 5)
print(top_5_good_deals)

```

Your answer:
~~~
We have house order number as 2181,2182,2044,2881,1783.These houses have the most negative percentage differences between their actual prices and the prices predicted by your model, indicating that they are potentially good deals based on the model's predictions.The negative percentage differences suggest that these houses are priced lower than what the model predicts, making them potential bargains.
~~~
...
predict_interval <- predict(lm_model, newdata = new_data, interval = "confidence", level = 0.95)
predict_interval
```
...
Yes, there is a relationship between mpg and horsepower, since the p-value is very small.

ii. How strong is the relationship between the predictor and the response?

The multiple R-squared value is 0.6059, indicating that approximately 60.59% of the variability in mpg is explained by horsepower.

iii. Is the relationship between the predictor and the response positive or negative?

The relationship is negative. This is evident from the negative coefficient estimate for horsepower (-0.157845). As horsepower increases, mpg tends to decrease.

iv. What is the predicted mpg associated with a horsepower of 98? What are the associated 95% confidence and prediction intervals?

The predicted mpg is 24.4678, and the 95% confidence interval is [23.97308,24.96108]
...
Your code:
```{r,echo=TRUE}
plot(Auto$horsepower, Auto$mpg, xlab = "Horsepower", ylab = "MPG", main = "Scatter Plot with Regression Line")
...
```{r,fig.width=9,fig.height=7,echo=show_code}
par(mfrow = c(2, 2))  
plot(lm_model, pch = 16, col = "blue")
title("Diagnostic Plots for Least Squares Regression Fit")
par(mfrow = c(1, 1))
...
Each scatterplot shows the relationship between two variables, and the diagonal contains histograms for each variable.
...
The overall F-statistic is highly significant (p-value < 2.2e-16), indicating a significant relationship between the predictors and the response variable (mpg). This suggests that the model is statistically significant in predicting the response.

ii. Which predictors appear to have a statistically significant relationship to the response?

The p-value of Intercept, Displacement, Weight, Year, and Origin are highly siginificant.

iii. What does the coefficient for the year variable suggest?

The coefficient for the year variable represents the expected change in mpg for a one-unit increase in the year, holding other variables constant.
...
```{r,fig.width=9,fig.height=7,echo=show_code}
par(mfrow = c(2, 2))  
plot(lm_model, 1, main = "Residuals vs Fitted")
plot(lm_model, 2, main = "Normal Q-Q Plot")
plot(lm_model, 3, main = "Scale-Location Plot")
plot(lm_model, 5, main = "Residuals vs Leverage")
par(mfrow = c(1, 1))
```
...
In the Residual vs Fitted plot, it seems like all points are around the horizontal line and no pattern.
In the QQ plot, we can see there are some points above the line which means they have larger errors than expected.
In the Scale-Location Plot, it seems like there is no pattern.
In the Residual vs Leverage, there are are several points outside the dashed line.
...
Your code:
```{r,echo=TRUE}
lm_model2 <- lm(mpg ~ weight+year+weight*year, data = Auto)
summary(lm_model2)
```

Your answer:
~~~
There is a relationship, as indicated by the significant p-values for the Intercept, weight, year, and the interaction term (weight:year).

The interaction term (weight:year) is added to account for the possibility that the effect of weight on mpg depends on the value of the year. In other words, the relationship between weight and mpg is not constant across different years.
The interaction term (weight:year) is statistically significant, as indicated by the low p-value (‘***’).
~~~
(f) Try a few different transformations of the variables, such as $log(X)$, $\sqrt{X}$, $X^2$. Comment on your findings.
```{r}
lm_model <- lm(log(mpg) ~ . - name, data = Auto)
summary(lm_model)
```
Your Findings
~~
The log-transformed model appears to be a better fit, with lower residuals and a higher R-squared value.
~~
...
~~~
Price and US are found to be significant predictors of Sales, while the presence of an Urban environment does not appear to have a significant effect. The model explains approximately 23.35% of the variability in Sales.
~~~
...
~~~
The intercept represents the estimated Sale is 13.04 when all predictor variables are zero. 
For each unit increase in Price, Sales are estimated to decrease by approximately 0.0545 units.
The coefficient for UrbanYes represents the change in Sales when the Urban variable changes from 0 (No) to 1 (Yes).However, this coefficient is not statistically significant (p = 0.936), so we don't draw meaningful conclusions about its impact on Sales.
The coefficient for USYes represents the change in Sales when the US variable changes from 0 (No) to 1 (Yes). 
~~~
...
~~~
Sales=13.04−0.0545×Price−0.0219×Urban+1.20×US
...
~~~
Therefore, we can reject the null hypothesis for the coefficients associated with Price Price and US, indicating that changes in these variables are associated with statistically significant changes in the Sales, while we cannot reject the null hypothesis for Urban.
...
~~~
The smaller model, considering only Price and US as predictors, still provides a good fit to the data, and both predictors are statistically significant in predicting Sales. The store's location in the US and the price of the car seat are associated with changes in Sales.
...
~~~
The Adjusted R-squared for the model in (a) is higher (0.8182) than the one in (e) (0.2354). This indicates that the model in (a), which includes Price, Urban, and US as predictors, fits the data better compared to the smaller model in (e), which only includes Price and US. The larger model explains a higher proportion of the variance in Sales, suggesting that the additional predictors in (a) contribute to a better fit.
...
```{r,fig.width=9,fig.height=7,echo=show_code}
par(mfrow = c(2, 2))  
plot(smaller_model, 1, main = "Residuals vs Fitted")
plot(smaller_model, 2, main = "Normal Q-Q Plot")
plot(smaller_model, 3, main = "Scale-Location Plot")
plot(smaller_model, 5, main = "Residuals vs Leverage")
par(mfrow = c(1, 1))
```
Your answer:
...
~~~
In the Residual vs Fitted plot, it seems like all points are around the horizontal line and no pattern.
In the QQ plot, we can see there are no points above the line 
In the Scale-Location Plot, it seems like there is no pattern.
In the Residual vs Leverage, there are are several points outside the dashed line.
